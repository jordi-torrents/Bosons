...
subroutine mesure_th(r, v, n, k, g, E_int, E_ho, E_kin, th)
  integer :: n, i, j
  real(8), dimension(n,2) :: r, v
  real(8) :: E_int, E_ho, E_kin, k, g, th, th2, d2, E_cut
  real(8), dimension(2) :: dif
  !f2py intent(out) E_int, E_ho, E_kin
  th2 = th*th
  E_cut = g/th2
...
subroutine verlet_steps_eps_3D(r, v, F, t, n, steps, k, g, dt, th, eps)
  integer :: n, steps, l
  real(8), dimension(n,3) :: r, v, F, F_new
  real(8) :: t, k, g, dt, d2, dt2, th, th2, eps
  real(8), dimension(3) :: dif, force_ij
  !f2py intent(inout) t
  ...
  do l=1,steps
    ...
    do i=1,n
      ...
      do j=i+1,n
        dif = r(j,:) - r(i,:)
        d2=dif(1)*dif(1)+dif(2)*dif(2)+dif(3)*dif(3) + eps
        if (d2<th2) then
          force_ij = -2.d0*g*dif/(d2*d2)
          F_new(i,:)=F_new(i,:)+force_ij
          F_new(j,:)=F_new(j,:)-force_ij
